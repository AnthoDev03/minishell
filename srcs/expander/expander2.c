...
void expand_env_var(t_expander *exp) {
  char var_name[256]; // Maximum size for an environment variable name. You can
                      // adjust this value.
  ...
  if (env_value) {
    size_t remaining_size =
        exp->buffer_size - (exp->write_pos - exp->expanded_str);
    while (remaining_size < strlen(env_value) + 1) {
      ptrdiff_t offset = exp->write_pos - exp->expanded_str;
      exp->buffer_size *= 2;
      exp->expanded_str = realloc(exp->expanded_str, exp->buffer_size);
      exp->write_pos = exp->expanded_str + offset;
      remaining_size = exp->buffer_size - (exp->write_pos - exp->expanded_str);
    }
